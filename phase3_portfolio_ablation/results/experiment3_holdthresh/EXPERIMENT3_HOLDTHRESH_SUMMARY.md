...
> 固定 pred + PortAna；输出 ARR、Cost Drag、IR、MDD、换手、平均持仓。
...
> 阈值：相邻 |ARR 差| > 3pp；参照 Exp2 持仓重叠排查流程。
...
> 整合 Q1（换手/成本单调）、Q2（ARR 稳定性 vs TopK/n_drop）、Q3（极端 ht=1 vs 30）。
